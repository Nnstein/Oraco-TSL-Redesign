...
import { Menu, X, Twitter, Linkedin, Facebook, Mail, MapPin, Sun, Moon, Home as HomeIcon, User, Briefcase, FileText, MessageSquare } from 'lucide-react';
import { Link, useLocation } from 'react-router-dom';
import Button from './Button';
import ChatWidget from './ChatWidget';
import AnimatedMobileMenu from './ui/animated-mobile-menu';
...
const Layout: React.FC<LayoutProps> = ({ children, navItems }) => {
  ...
  return (
    <div className="min-h-screen flex flex-col bg-white dark:bg-slate-950 transition-colors duration-300 overflow-x-hidden selection:bg-brand-primary/20 selection:text-brand-dark dark:selection:text-white">
      {/* Navbar */}
      <header
        className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${isScrolled || isMobileMenuOpen ? 'glass-nav py-2 md:py-3' : 'bg-transparent py-3 md:py-5'
      ...
      >
        <div className="container mx-auto px-4 md:px-6 flex items-center justify-between">
            ...
            <nav className="flex items-center gap-1 bg-white/50 dark:bg-slate-900/50 backdrop-blur-sm px-2 py-1.5 rounded-full border border-slate-100 dark:border-slate-800 shadow-sm transition-colors">
              {navItems.map((item) => (
                <Link
                  key={item.path}
                  to={item.path}
                  className={`px-3 lg:px-5 py-2 text-sm font-medium rounded-full transition-all duration-300 ${location.pathname === item.path
          ...
          <div className="flex items-center gap-3 md:hidden relative z-50">
            <AnimatedThemeToggle isDark={isDark} onToggle={toggleTheme} className="scale-90" />
            <AnimatedMobileMenu
              menuItems={[
